...
while ($null -ne ($line = [Console]::In.ReadLine())) {
  ...
  try {
    switch ($p[0]) {
      ...
      'hidebyport' { $c = Get-NetTCPConnection -LocalPort ([int]$p[1]) -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1; if ($c) { $w = Get-Process -Id $c.OwningProcess -ErrorAction SilentlyContinue; if ($w -and $w.MainWindowHandle -ne 0) { [OHWin]::ShowWindow([IntPtr]$w.MainWindowHandle, 0) | Out-Null; Write-Output ('hid:' + $w.MainWindowHandle) } } }
