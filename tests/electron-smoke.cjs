...
app.whenReady().then(async () => {
  ...
  const targetDialog = await win.webContents.executeJavaScript(`(() => {
    ...
    return { left: rect?.left || 0, top: rect?.top || 0, width: rect?.width || 0, height: rect?.height || 0, icons: document.querySelectorAll('.harness-target-dialog .target-card-icon').length, cards: document.querySelectorAll('.harness-target-dialog .target-card').length, hasInstalledLabel: [...document.querySelectorAll('.harness-target-dialog .target-card-top')].some((item) => item.textContent.includes('已安装')), checkboxRight: checkbox?.right || 0, cardRight: card?.right || 0, iconTop: icon?.top || 0, cardTop: card?.top || 0, statusBottom: status?.bottom || 0, statusIconTop: dshIcon?.top || 0, viewportWidth: innerWidth, viewportHeight: innerHeight }
  })()`)
