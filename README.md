...
**扫描本机**只扫描同步目录的直接子目录（忽略隐藏目录、插件缓存和已管理链接），扫描结果会持久保留，直到用户点击“刷新”或再次扫描；导入时复制并保留原目录；已有同名目录不会自动接管或覆盖。**更新**重新读取本地来源或 Git 默认分支，替换管理库内容；源文件应保留在独立目录中。**删除**移除管理库副本和指向它的同步链接，不删除原始导入目录。单个 Skill 限 100 MB / 10000 个文件，内部链接和特殊文件会被拒绝。
