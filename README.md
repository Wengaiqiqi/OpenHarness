...
侧栏打开 **Skills**，首页按 Harness 展示已同步 Skill（卡片固定高度，超出部分可在详情中查看）。进入 **Skill 管理** 后，点击 **扫描本机** 查找同步目录里的 Skill，勾选结果并点击 **导入 Skill**，再选择一个或多个目标 Harness。扫描结果和管理库都支持搜索；原始目录不会被修改。也可以在管理页手动导入包含 `SKILL.md` 的本地文件夹，或填写 HTTPS Git 仓库地址和 Skill 子目录。Git 导入需要已安装 Git，读取默认分支；不执行 Skill 脚本。管理库位于应用 `userData/skills`，可从 Skill 详情打开完整文件夹。
